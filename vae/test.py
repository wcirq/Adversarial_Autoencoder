...
for epoch in range(100):
    ...
    z = tf.random.normal((batchsz, h_dim))  # 从正太分布中sample这个尺寸的
    logits = model.decoder(z)  # 通过这个得到decoder
    x_hat = tf.sigmoid(logits)
    x_hat = tf.reshape(x_hat, [-1, 28, 28]).numpy() * 255.
    logits = x_hat.astype(np.uint8)  # 标准的图片格式；
    save_images(x_hat, 'ae_images/sampled_epoch%d.png' % epoch)  # 直接sample出的正太分布；

    # evaluation 2: 正常的传播过程；
    x = next(iter(test_db))
    x = tf.reshape(x, [-1, 784])
    x_hat = model(x)  # 前向传播返回的还有mu, log_var
